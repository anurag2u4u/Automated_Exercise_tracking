...
#creating list of names if column
...
def mark_outliers_chauvenet(dataset, col, C=2):
    ...
    higher values of c make outlier detection stricter,meaning fewer data points will be flagged.
    the default value is 2 which is commonly used.
    Returns:
        pd.DataFrame: The original dataframe with an extra boolean column 
        indicating whether the value is an outlier or not.
    """
    ...
    std = dataset[col].std()
    N = len(dataset.index) #no. of rows
    """criterion: This is the threshold value for identifying outliers. 
    The smaller this value, the more likely data points far from the mean will be
    classified as outliers. It is calculated as 1 / (C * N), where C is 
    the degree of certainty (default is 2) and N is the dataset size"""
    criterion = 1.0 / (C * N) 
    ...
    prob = [] #a list to store the probability that a data point is consistent with normal distribution.
    mask = [] #a list to store boolean values indicating whether each point is an outlier.
...
# --------------------------------------------------------------
"""
This code detects outliers using the Local Outlier Factor (LOF) method, 
which is an unsupervised outlier detection algorithm.
The LOF algorithm compares the local density of each data point to 
the densities of its neighbors to identify points that
have a significantly lower density (indicating they are outliers).
also look how KNN works.
